                ...
                $user = $q->row();
                            ...
                            $result = $gcm->send_notification(array($user->user_ios_token),$message ,"ios");
                                ...
                                $message = '<div class="container">
                                 ...
                                 <p>Your order has confirmed by admin</h1>
                                 </div>';
                                $message = 'message of email';
                               
                                $subject = 'Your Order has Confirmed By admin';
...
where sale_id = '".$order_id."'"); 
                ...
                $this->session->set_flashdata("message",'<div class="alert alert-success alert-dismissible" role="alert">
                                ...
                                $to = $user_email;
                                $subject = 'Your Order has Delivered';
                                $message = '<div class="container">
                                <h1>Congratulations</h1>
                                <p>Your order has confirmed by admin</h1>
                                </div>
        ...
        if(_is_user_login($this)){
            ...
            if(!empty($order)){
                ...
                $this->session->set_flashdata("message",'<div class="alert alert-success alert-dismissible" role="alert">
                                ...
                                $message = '<div class="container">
                                <h1>Congratulations</h1>
                                <p>Your order has canceled by admin</h1>
                                </div>
                                ';
    ...
    public function Product_reviews(){
        ...
        $data= array();
        $data['reviews'] = $this->product_model->review_list();
        
        $this->load->view("admin/Product_review/product_review_list2",$data); 
    ...
    public function Review_list($id=0){
        ...
        $data['reviews'] = $this->product_model->review_by_id($id);
           $this->load->view('admin/Product_review/review_list',$data);
    }
    public function delete_review($id1){
        $this->load->helper('form');
        $this->load->model('product_model');
        $this->load->library('session');

     if($this->product_model->delete_review_by_id($id1)){
        
        $data['reviews'] = $this->product_model->review_by_id($this->session->userdata('product_id'));
        $this->load->view('admin/Product_review/review_list',$data);
     }
     else {
        $data['reviews'] = $this->product_model->review_by_id($id);
    ...
    public function approve_review($id){
        if($this->product_model->approved_review_by_id($id)){
            echo 'approved successfully';

        }
        else {
            echo 'unable to approved';
        }
    }

    public function unapprove_review($id){
       
        if($this->product_model->unapproved_review_by_id($id)){
            echo 'Unapproved successfully';

        }
        else {
            echo 'unable to Unapproved';
        }
       ...
       if(_is_user_login($this)){
            ...
            if(isset($_POST))
            {
                ...
                else
                {
                    ...
                    "State_code"=>$this->input->post("product_location")
       ...
       if(_is_user_login($this)){
            ...
            if(isset($_POST))
            {
                ...
                else
                {
                    ...
                    "tax"=>$this->input->post("tax"), 
                    "rewards"=>$this->input->post("rewards"),
                    "product_pincode"=>$this->input->post("pincode"),
                    'net_weight' =>$this->input->post("weight")
                    );
                    if($_FILES["prod_img"]["size"] > 0){
                        $config['upload_path']          = './uploads/products/';
                        $config['allowed_types']        = 'gif|jpg|png|jpeg';
                ...
                $this->form_validation->set_rules('descri', 'Description', 'trim|required');
                  if ($this->form_validation->run() == FALSE)
                  {
                      ...
                      $message["title"] = $this->config->item("company_title");
                                ...
                                $message["image"] = "http://neerajbisht.com/grocery_test/uploads/signature/IMG-20180617-WA0003.jpg";
                                $message["created_at"] = date("Y-m-d h:i:s");  
...
function delete_deal_product($id){
    ...
    public function move(){
        ...
        $curl = curl_init();
        curl_setopt ($curl, CURLOPT_URL, "http://384772.true-order.com/WebReporter/api/v1/items");
        ...
        foreach ($data as $key => $item){
            echo 'Value found at array key <br><br> ' . $key;
        }
        
         //$q=json_decode($projects);
         //echo $q[0]['itemid'];
...
function add_offer(){
    ...
    $this->form_validation->set_rules('product_id', 'Offer Title', 'required');
    ...
    $data= array();
    $data['coupons'] = $this->product_model->offer_list();
    if($this->form_validation->run() == FALSE)
    {
        ...
        $data['query1']=$this->db->get()->result();
        
        $this->load->view("admin/offers/add_offers",$data); 
         
    }else{
        $data = array(
        'offer_name'=>$this->input->post('offer_title'),
        'product_id'=> $this->input->post('product_id'),
        ...
        'discount'=> $this->input->post('discount'),
         ...
         if($this->product_model->offer($data))
         {
            ...
            // $from = 'Gopalsh98598@gmail.com';
            // $to = $user_email1;
            // $subject = 'You Get A Offer By Admin';
            // $message = '<div class="container" style="text-align:center;margin:auto;width:50%;height:245px; background-color:rgba(0,0,0,0.1);border-radius:12px;">
            // <h1>Congratulations</h1>
            // <h3>You Are Getting A Product '.$product_name.' with '.$data['discount'].'% of discount </h3>
            // <p>This Offer is Valid for '.$data['from'].' to '.$data['to'].'</p>
            // <a href="https://alphawizz.org/sweets_ecommerce">Click To Buy</a>
            // </div>
            // ';
            ...
            $this->session->set_flashdata('addmessage','offer added Successfully.');
            $data['coupons'] = $this->product_model->offer_list();
            $this->load->view("admin/offers/offer_list2",$data);
         }
    }
   //$data['coupons'] = $this->product_model->coupon_list();
...
function editOffer($id){
   ...
   $this->load->model('product_model');
    ...
    {
        $this->form_validation->set_error_delimiters('<div class="text-danger">', '</div>');
        $data['offer'] = $this->product_model->getOffer($id);
        ...
        'to'=> $this->input->post('to'),
         ...
         print_r($data);
         if($this->product_model->editOffer($id,$data)){
            $this->session->set_flashdata('addmessage','Coupon edited Successfully.');
...
{
    $this->load->model('product_model');
    if($this->product_model->deleteOffer($id))
    {
        $this->session->set_flashdata('addmessage','One Coupon deleted Successfully.');
        redirect("admin/offers");
    }
